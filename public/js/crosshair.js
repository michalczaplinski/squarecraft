...
function Crosshair() {
	...
	for (var iPetal = 0; iPetal < petals; iPetal++) {
		...
		geometry.vertices.push(new THREE.Vector3(Math.cos(angle) * radiusInner, Math.sin(angle) * radiusInner, 0));
		geometry.vertices.push(new THREE.Vector3(Math.cos(angle) * radiusOuter, Math.sin(angle) * radiusOuter, 0));
